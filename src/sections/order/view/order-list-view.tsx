...
export default function OptionChainPage() {
  ...
  const fetchOptionChainFromLTP = useCallback(async () => {
    try {
      ...
      const apiUrl =
        symbol === "NIFTY"
          ? "http://localhost:4000/api/nifty/option-chain?ltp=25000"
          : "http://localhost:4000/api/nifty/option-chain?symbol=BANKNIFTY&ltp=52000";
 ...
 const placeOrder = async (opt: OptionItem, side: "BUY" | "SELL") => {
  try {
    ...
    // ✅ save order to DB
    await fetch("http://localhost:4000/api/orders/save", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        clientcode: "ANBG1133",
...
const checkOrderStatus = async (orderid: string) => {
  const res = await fetch(
    `http://localhost:4000/api/orders/status/ANBG1133/${orderid}`
  );
  const json = await res.json();
